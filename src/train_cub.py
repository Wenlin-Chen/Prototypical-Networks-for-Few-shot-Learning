...
from cub_model import GoogLeNetExtension
...
def train(tr_dataloader, model, loss_fn, optimizer, lr_scheduler, val_dataloader, device):
    ...
    for epoch in range(params.epochs):
        ...
        for batch in tr_iter:
            ...
            bs, ncrops, c, h, w = x.size() # [500, 10, 3, 224, 224]
            x2 = x.view(-1, c, h, w) # fuse batch size and ncrops
            print('x2 size', x2.size()) # [5000, 3, 224, 224]

            # model is GoogLeNetExtension (pretrained before pooling layer + pooling)
            x_embed = model(x2) 
            print('embedding size: ', x_embed.size()) # [batch_size, 1024, 1, 1]
...
def run():
    ...
    # Pretrained GoogLeNet before pooling layer + pooling
    model = GoogLeNetExtension()
    if device == 'cuda:0':
        model.to('cuda')
